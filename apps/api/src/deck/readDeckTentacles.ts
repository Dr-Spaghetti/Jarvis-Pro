...
import { readBundledCatalogSkills } from "../skillsCatalog";
...
export const readDeckVaultFile = (
  ...
  // Prevent path traversal (including Windows backslash separator)
...
export const listDeckAvailableSkills = (workspaceCwd: string): DeckAvailableSkill[] => {
  // Project/user skills (real copies in `.claude/skills/`) take precedence over
  // bundled catalog skills with the same name.
  const installed = readAvailableClaudeSkills(workspaceCwd);
  const installedNames = new Set(installed.map((skill) => skill.name));
  const bundled = readBundledCatalogSkills().filter((skill) => !installedNames.has(skill.name));
