...
const startServer = async () => {
  // Load a project-local .env so bundled-skill API keys reach every agent the
  // PTY runtime spawns (it forwards process.env verbatim).
...
const skillsList = async () => {
  loadEnvFile(process.cwd());
  const { listCatalogSkills } = await import("./skillsCatalog");
  const { readAvailableClaudeSkills } = await import("./claudeSkills");

  const bundled = listCatalogSkills();
  const installed = readAvailableClaudeSkills(process.cwd());

  if (bundled.length === 0 && installed.length === 0) {
  ...
  if (installed.length > 0) {
    ...
    for (const skill of installed) {
      ...
      console.log(`  ${skill.name} [${skill.source}]${description}`);
    }
    console.log();
  }

  if (bundled.length > 0) {
    console.log("Bundled skills (always available inside Octogent):");
    for (const skill of bundled) {
      const description = skill.description ? ` — ${skill.description}` : "";
      const missing =
        skill.missingEnv && skill.missingEnv.length > 0
          ? `  (needs ${skill.missingEnv.join(", ")})`
          : "";
      console.log(`  ${skill.name}${description}${missing}`);
    }
    console.log();
    console.log("Install one into this project with: octogent skills install --skill <name>");
  }
};

const skillsInstall = async () => {
  const { installCatalogSkills } = await import("./skillsCatalog");

  const installAll = hasFlag("--all");
  const skillName = parseFlag("--skill") ?? parseFlag("-s");
  const force = hasFlag("--force");

  if (!installAll && !skillName) {
    console.error("Error: specify --all or --skill <name>.");
    process.exit(1);
  }

  const result = installCatalogSkills(process.cwd(), {
    ...(installAll ? {} : { names: [skillName as string] }),
    force,
  });

  if (!result.catalogFound) {
    console.error("Error: bundled skills catalog could not be located.");
    process.exit(1);
  }
  ...
  for (const skipped of result.skipped) {
    console.log(`Skipped ${skipped.name} (${skipped.reason}; use --force to overwrite)`);
  }
  for (const failure of result.errors) {
    console.error(`Failed ${failure.name}: ${failure.error}`);
  }
...
const skillsStatus = async () => {
  ...
  console.log("Bundled skill readiness:");
  for (const skill of bundled) {
    const requiredEnv = skill.requiredEnv ?? [];
    if (requiredEnv.length === 0) {
      console.log(`  ${skill.name}: ready (no API key required)`);
      continue;
    }
    const missing = skill.missingEnv ?? [];
    if (missing.length === 0) {
      console.log(`  ${skill.name}: ready (${requiredEnv.join(", ")} set)`);
    } else {
...
const skillsDoctor = async () => {
  ...
  for (const skill of bundled) {
    for (const key of skill.requiredEnv ?? []) requiredEnvKeys.add(key);
    for (const runtime of skill.runtime) runtimes.add(runtime);
  }

  console.log("Octogent skills doctor");
  console.log();
  console.log(`  node: ${process.version}`);
  console.log(`  catalog skills: ${bundled.length}`);
  console.log();

  console.log("  API keys:");
  if (requiredEnvKeys.size === 0) {
    console.log("    (none required)");
  } else {
    for (const key of [...requiredEnvKeys].sort((a, b) => a.localeCompare(b))) {
  ...
  for (const skill of bundled) {
    if (skill.setup) {
      console.log(`    ${skill.name}: ${skill.setup}`);
    }
  }
  console.log();
  console.log("  Add missing keys to a .env file in your project root.");
};
...
const main = async () => {
  ...
  octogent skills list                 List bundled and installed skills
  octogent skills install --all        Copy all bundled skills into .claude/skills
  octogent skills install --skill <n>  Copy one bundled skill into .claude/skills
    --force                            Overwrite an already-installed skill
  octogent skills status               Show which bundled skills have their API keys set
  octogent skills doctor               Diagnose skill prerequisites (keys, runtimes, setup)`);
  process.exit(1);
};
