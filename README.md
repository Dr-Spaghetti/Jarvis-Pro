...
> **Octogent + Skills.** This build combines [Octogent](https://github.com/hesamsheikh/octogent) (the orchestration dashboard) with the [rm-skills](https://github.com/reymerekar7/rm-skills) catalog, so every tentacle agent ships with a ready-to-use library of practical skills. See [Credits](CREDITS.md).

## Bundled Skills

Octogent ships with a **bundled skills catalog** — a set of
[Claude Code skills](https://docs.claude.com/en/docs/claude-code/skills) every
tentacle agent can use. Bundled skills are **always available inside the
dashboard** (tagged `bundled`) and are read in place — running Octogent in a repo
never copies skill files into it unless you explicitly run `octogent skills
install`.
...
```bash
octogent skills list                 # bundled + installed skills, with missing keys flagged
octogent skills status               # which bundled skills have their API keys set
octogent skills doctor               # diagnose keys, runtimes, and setup commands
octogent skills install --skill <name>   # copy a skill into this project's .claude/skills/
octogent skills install --all            # copy them all
```

Put API keys in a `.env` at your project root (copy [`.env.example`](.env.example)).
Octogent loads it at startup and forwards every variable to the agents it spawns,
so one keystore powers all tentacles. Full details: [Skills Catalog](docs/guides/skills-catalog.md).
...
- [Skills Catalog](docs/guides/skills-catalog.md)
